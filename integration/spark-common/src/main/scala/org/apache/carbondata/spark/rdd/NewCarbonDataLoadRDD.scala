 ...
 *  @see org.apache.carbondata.processing.newflow.DataLoadExecutor
 */
class NewInternalRowLoaderRDD[K, V](
    @transient private val ss: SparkSession,
    result: DataLoadResult[K, V],
    carbonLoadModel: CarbonLoadModel,
    prev: DataLoadCoalescedRDD[InternalRow]) extends CarbonRDD[(K, V)](ss, prev) {
  ...
  override def internalCompute(theSplit: Partition, context: TaskContext): Iterator[(K, V)] = {
    ...
    val iter = new Iterator[(K, V)] {
      ...
      val executionErrors = new ExecutionErrors(FailureCauses.NONE, "")
      val model: CarbonLoadModel = carbonLoadModel
      // TODO: For internal row, no need of converter step, have just direct dictionary converter
