...
object CarbonDataRDDFactory {
  ...
  def insertCarbonDataWithDf(sqlContext: SQLContext,
      carbonLoadModel: CarbonLoadModel,
    ...
    val columnar = sqlContext.sparkSession.conf.get("carbon.is.columnar.storage", "true").toBoolean
    ...
    try {
      if (!carbonLoadModel.isCarbonTransactionalTable || segmentLock.lockWithRetries()) {
        if (updateModel.isDefined) {
          ...
          status = if (isSortTable && sortScope.equals(SortScopeOptions.SortScope.GLOBAL_SORT)) {
            DataLoadProcessBuilderOnSpark.insertDataUsingGlobalSortWithDF(sqlContext.sparkSession,
              dataFrame,
              carbonLoadModel,
              hadoopConf)
          } else {
  ...
  def insertCarbonData(sqlContext: SQLContext,
      carbonLoadModel: CarbonLoadModel,
      partitionStatus: SegmentStatus = SegmentStatus.SUCCESS,
      overwriteTable: Boolean,
      hadoopConf: Configuration,
    ...
    val convertedRdd = CommonLoadUtils.getConvertedInternalRow(colSchema, scanResultRDD)

    try {
      if (!carbonLoadModel.isCarbonTransactionalTable || segmentLock.lockWithRetries()) {
        status = if (isSortTable && sortScope.equals(SortScopeOptions.SortScope.GLOBAL_SORT)) {
          DataLoadProcessBuilderOnSpark.insertDataUsingGlobalSort(sqlContext.sparkSession,
            convertedRdd,
            carbonLoadModel,
            hadoopConf)
        } else {
          loadRdd(sqlContext, convertedRdd, carbonLoadModel)
        }
        val newStatusMap = scala.collection.mutable.Map.empty[String, SegmentStatus]
        if (status.nonEmpty) {
          status.foreach { eachLoadStatus =>
  ...
  private def loadRdd(
    ...
    try {
      val nodeNumOfData = scanResultRDD.partitions.flatMap[String, Array[String]] { p =>
        DataLoadPartitionCoalescer.getPreferredLocs(scanResultRDD, p).map(_.host)
      }.distinct.length
      val nodes = DistributionUtil.ensureExecutorsByNumberAndGetNodeList(
        nodeNumOfData,
        sqlContext.sparkContext)
      val newRdd = new DataLoadCoalescedRDD[InternalRow](sqlContext.sparkSession,
        scanResultRDD,
        nodes.toArray
          .distinct)

      new NewInternalRowLoaderRDD(
        sqlContext.sparkSession,
        new DataLoadResultImpl(),
        carbonLoadModel,
        newRdd
