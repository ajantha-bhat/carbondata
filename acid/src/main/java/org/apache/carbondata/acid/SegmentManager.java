...
//  private Map<String, SegmentStatusReader> reader = new HashMap<>();
//  private Map<String, SegmentStatusUpdater> updater = new HashMap<>();

  /**
   * Get the SegmentStatusReader instance to read the segment metadata files (table status file, segment file, update status files). There is no lock acquired during read operation.
  ...
  public SegmentStatusReader getSegmentStatusReader(AbsoluteTableIdentifier identifier) {
    // TODO: new object everytime, no need of map
    return null;
  }
